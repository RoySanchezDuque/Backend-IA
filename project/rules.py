...
try:
    ...
    def print_tree(node_id, depth):
        ...
        if feature_idx != -2:  # Not a leaf
            ...
            indent = "  " * depth
            sys.stdout.write(f"{indent}if {feature_name} <= {threshold_original:.4f}:\n")
            print_tree(tree.children_left[node_id], depth + 1)
            sys.stdout.write(f"{indent}else:  # {feature_name} > {threshold_original:.4f}\n")
            print_tree(tree.children_right[node_id], depth + 1)

    sys.stdout.write("Decision Tree Rules (Depth 3):\n")
    print_tree(0, 0)
